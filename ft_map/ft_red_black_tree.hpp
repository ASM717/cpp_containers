...
namespace ft {
	...
	public:
		...
		explicit RBTree(P data) : _alloc(std::allocator<node >()), root(_alloc.allocate(1)), elem(NULL), _comp(std::less<Key >()) {
		...
		RBTree(const RBTree& x) : _alloc(x._alloc), root(NULL), elem(NULL), _comp(std::less<Key >()) {
			*this = x;
		}
